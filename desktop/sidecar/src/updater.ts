...
export class AppUpdater {
  ...
  private async checkForUpdateResult(includeBeta: boolean): Promise<{
    ...
    const apiUrl = 'https://api.github.com/repos/tscodeplus/DeepSeek Harness/releases?per_page=30';
    ...
    const ymlName =
      process.platform === 'win32'
        ...
        : process.platform === 'darwin'
          ? 'latest-mac.yml'
          : 'latest-linux.yml';
    const latestYmlUrl = `https://github.com/tscodeplus/DeepSeek Harness/releases/download/${release.tag_name}/${ymlName}`;
  ...
  private async downloadFromPendingUpdate(): Promise<void> {
    ...
    const downloadUrl = `https://github.com/tscodeplus/DeepSeek Harness/releases/download/v${update.version}/${fileInfo.url}`;
  ...
  async installUpdate(): Promise<void> {
    ...
    if (this.isMacOSUnsigned()) {
      ...
      void shellFetch('/open-external', { url: 'https://github.com/tscodeplus/DeepSeek Harness/releases' });
      return;
    }
...
<script>
  ...
  private async runNetworkDiagnostic(): Promise<void> {
    const testUrls = [
      { label: 'GitHub API', url: 'https://api.github.com/repos/tscodeplus/DeepSeek Harness/releases/latest' },
      { label: 'latest.yml (beta3)', url: 'https://github.com/tscodeplus/DeepSeek Harness/releases/download/v2.0.0-beta3/latest.yml' },
    ];
